...
| gated-K | `gate_bank_gated_k: true` (needs include_k) | `bank_k_diag` + `k2_w` | K likewise; identity init (diag=0, b=1.27846 → gate≡1): step 0 == ungated arm; verified BIT-IDENTICAL vs ungated same-seed pb; cost −5.9% at h10 (4153 vs 4414) |

**gated-QK: TESTED, REJECTED (2026-06-12).** Fresh-run training arm underperformed the ungated QK baseline — the net does not want higher-order addressing from shared-bank gates, at least from scratch. Combined with the −5.9% inference cost, the arm is dead. DO NOT RE-SUGGEST order-raising spends on the Q/K addressing pathway without new evidence; this updates the break-linearity pattern: order increases won on CONTENT/gating pathways (GLU-V, SwiGLU, VGA-E, smolgen), but addressing declined the upgrade. Code stays (presence-detected, dormant); the only sanctioned revival is the cheap identity-init FINE-TUNE probe on a mature QK checkpoint + the weight-space drift readout (|diag|, |bias−1.278| per layer) before any fresh-run retry. Capacity budget redirects to content/heads: value residual (~free), head beef (~1%), dict r=8 residual (pending kernel ILP work).
...
**Speed state of the h10+QK+dict production arm (2026-06-12).** Fresh profile (h10dict-prof.*): 640-col GEMMs 42.3%, small-GEMM cluster (K2/V-up/decoder/QKᵀ) 11.9%, rms_norm 7.3% (at roofline, residual-add already fused), expandKV 7.1%→FIXED, bank gates 7.1% (roofline), A·V+softmax 8.5%, VGA 3.7% (roofline). Stream busy at capture-off: main 66% / ffn 20% / smol 14% of span — partially launch/sync-bound, gaps not compute. **expandKVWeighted v2 landed (c051fdb): smem-staged one-block-per-token, bit-identical, kernel 48→32.7µs, end-to-end +5.5% (interleaved ×3: 4169 vs 3953 — a single non-interleaved pair showed v2 LOSING 2% from thermal drift; NEVER trust single pairs on this box).** LC0_EXPANDKV_V1=1 = old kernel escape. Dict at h10 = speed-NEUTRAL (4373 vs 4413, both machines agree): heads-10 pushed smolgen off the critical path; dict's value = −32M params at equal speed; smolgen_dict_atoms: 128 likely free now. **DEFERRED: [Q2|K2] fused h-GEMM (+1-1.5% est)** — design: concat (960,640) weight like mha_q1k1_w_; output carved at nla_qk_temp+2 slabs (bump reservation 6→7*qkv for Layout-1+K-on-bank nets); K side free (expandKV k_in_stride=960); BLOCKER is Q at stride 960: genOffsetPointers needs a q_stride param (cached ONCE across layers — all layers uniform ✓), QKᵀ batched ldb 960, strided Q2-bias kernel, gate on !gated/!exo. ~10 edit sites; wrong-stride bugs = legal-but-wrong chess. Remaining honest ceiling after it: ~+1.5%, then fp16 is mined out (int8 QAT is the only big lever left and is shelved).
